function approved_by_the_FD() {
  try {
    ...
    let emailBody = `Реестр на ${day} согласован.\n`;
    emailBody += `Несогласованных платежей - ${unsettledCount}, строк в реестре - ${countC}\n`;
    emailBody += `Сумма всех платежей - ${formattedSum} руб.\n\n`;

    // Добавляем информацию о несогласованных платежах если они есть
    if (unsettledCount > 0) {
      emailBody += "Несогласованные платежи:\n" + unsettledPayments.join("\n") + "\n\n";
    } else {
      emailBody += "Все платежи согласованы.\n\n";
    }
    ...
    MailApp.sendEmail({
      // to: "pankov@steit.ru", // Раскомментируйте нужные email-адреса
    ...
    notifyFinGroup(
      `Реестр на ${currentDate} ФД согласован. Строк в реестре: ${countC}, сумма: ${formattedSum} руб.`
    );
...
// Вспомогательная функция для получения последней заполненной строки
...
// Функция для экспорта листа в XLSX
